"""
Webots controller — Phase 1.1: same fixed spawn position as phase 1, but the
robot’s heading (yaw around Z) is randomized every episode. The agent must
learn to rotate and search for the red ball instead of always starting aimed
at it.

Same observation/action/reward structure as phase_1_initial_training.py so
weights remain compatible; use phase_1_trained_controller-style inference after
training by pointing MODEL_PATH at the saved checkpoint.
...
ROBOT_START_POSITION = [-1.1, 0.019, 0.0]   # [x, y, z] in meters (fixed every episode)
# Yaw is sampled on each reset; Webots rotation = [axis_x, axis_y, axis_z, angle_rad].
...
class RoombaRedBallEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        1. Stop motors
        2. Teleport robot to ROBOT_START_POSITION with a random yaw (Z axis)
        ...
        # Teleport robot to starting position using supervisor API (fixed XY; random yaw)
        robot_translation_field.setSFVec3f(ROBOT_START_POSITION)
        yaw = float(self.np_random.uniform(-np.pi, np.pi))
        robot_rotation_field.setSFRotation(ROBOT_YAW_AXIS + [yaw])
