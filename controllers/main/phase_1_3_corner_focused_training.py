...
class RoombaRedBallEnv(gym.Env):
    ...
    def step(self, action):
        ...
        for _ in range(FRAME_SKIP):
            ...
            # Direction-agnostic rise: rear-wall ramming when reversing is just as
            # bad as front-wall ramming when going forward.
            sensor_rise = max(0.0, sensor_max - self.previous_sensor_max)
            self.step_count += 1
            ...
            # the same as forward motion (otherwise the agent learns to scan in reverse).
            lin_norm = float(np.clip(abs(linear_velocity) / MAX_SPEED, 0.0, 1.0))

            # --- Rising proximity (any direction) while not clearly seeing the ball ---
            if not touching_ball and self.current_red_pixel_ratio < FRONT_RISE_RED_MAX:
                reward -= FRONT_RISE_COEFF * sensor_rise
            ...
            if wedged_blind:
                ...
                # Direction-agnostic: spin bonus only when the robot is actually
                # slow (forward OR reverse), not whenever lin is negative.
                if abs(linear_velocity) < SPIN_BONUS_MAX_LINEAR_FRAC * MAX_SPEED:
